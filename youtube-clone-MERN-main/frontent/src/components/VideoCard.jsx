...
const VideoCard = ({ video, className = "" }) => {
  const { id, title, description, thumbnail, duration, views, createdAt, channel } = video;

  // Fallback handlers
  const handleThumbnailError = (e) => (e.target.src = getVideoThumbnail(id));
  const handleAvatarError = (e) => (e.target.src = getUserAvatar(channel?.id));

  return (
    <div
      className={`bg-white rounded-lg overflow-hidden shadow-sm hover:shadow-md transition-all duration-300 animate-fade-in ${className}`}
    >
      {/* Thumbnail */}
      <Link to={`/watch/${id}`} className="block relative group">
      ...
      <div className="p-3">
        <div className="flex space-x-3">
          {/* Channel Avatar */}
          <Link to={`/channel/${channel?.id}`} className="flex-shrink-0 hover:opacity-80 transition-opacity">
            <img
              src={channel?.avatar || getUserAvatar(channel?.id)}
              alt={channel?.name || "Channel"}
              className="w-9 h-9 rounded-full object-cover"
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center justify-between">
              ...
              {/* Subscribe Button */}
              <button className="ml-2 px-3 py-1 text-xs font-semibold bg-red-600 text-white rounded hover:bg-red-700 transition">
                Subscribe
              </button>
            </div>
            ...
            <div className="flex items-center space-x-2 text-sm text-gray-500">
              {/* Views as button */}
              <button className="px-2 py-0.5 text-xs bg-gray-100 rounded hover:bg-gray-200 transition">
